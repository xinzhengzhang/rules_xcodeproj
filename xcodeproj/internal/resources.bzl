...
def _process_resource(
    ...
    if not file.is_source:
        ...
        if bundle_path and file.basename == "Info.plist":
            ...
            # TODO: Find a way to include the original Info.plist and the
            #   generated one in the Xcode project. The generated file is under
            #   a ".bundle/" path, which triggers Folder Type detection. The
            #   original file can't reliably be found with the data in
            #   `AppleResourceInfo` as it currently stands.
            return None
...
def collect_resources(
    ...
    if str(target.label) == "//container:Spotify":
        print("\nFor Brentley:\n", resource_info)
