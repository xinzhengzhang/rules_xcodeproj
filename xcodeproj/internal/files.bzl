...
def file_path(file, path = None, is_folder = False):
    ...
    if not file.is_source:
        return generated_file_path(path, is_folder = is_folder)
    if file.owner.workspace_name:
        return external_file_path(path, is_folder = is_folder)
    return project_file_path(path, is_folder = is_folder)
...
def _file_path(type, *, path, is_folder):
    return struct(
        path = path,
        type = type,
        is_folder = is_folder,
...
def external_file_path(path, is_folder = False):
    return _file_path(
        # Type: "e" == `.external`
        type = "e",
...
def generated_file_path(path, is_folder = False):
    return _file_path(
        # Type: "g" == `.generated`
        type = "g",
...
def project_file_path(path, is_folder = False):
    return _file_path(
        # Type: "p" == `.project`
        type = "p",
        path = path,
